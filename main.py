...
def print_portfolio_overview(state: PortfolioState) -> None:
    """输出 Schema 1.1 只读持仓概览，不请求实时行情。"""
    print("\n=== 持仓概览（Schema 1.1，只读）===")
...
def show_portfolio_overview(path: str | Path) -> bool:
    ...
    try:
        ...
        print(f"\n[错误] 持仓概览无法读取：{exc}")
        return False
    print_portfolio_overview(state)
    return True
...
def main():
    ...
    if args.command == "dashboard":
        ...
        if args.add or args.sell is not None or args.sync or args.config:
            ...
            show_portfolio_overview(args.portfolio_file)
