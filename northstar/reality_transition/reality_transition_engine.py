...
v9: Multi-Strategy Alpha Allocation System (MSAAS).
策略级资本分配 → Meta Allocator → 最终组合。
...
class RealityTransitionEngine:
    ...
    def compute_strategy_allocation(self, profiles: list[dict[str, Any]], regime: str) -> dict[str, Any]:
        ...
        for p in profiles:
            if p["risk"] > 0:
                raw[p["name"]] = p["rmai"] * max(p["sharpe"], 0.01) * p["regime_fit"] / p["risk"]
            else:
                raw[p["name"]] = 0
        tr = sum(raw.values())
        weights = {k: v / tr for k, v in raw.items()} if tr > 0 else {k: 1.0 / len(profiles) for k in profiles}
        max_single = 0.15 if regime == "liquidity_stress" else MAX_SINGLE_STRATEGY
        if regime == "liquidity_stress":
            for k in weights: weights[k] *= 0.3
        for k in list(weights.keys()):
            if weights[k] > max_single:
                excess = weights[k] - max_single; weights[k] = max_single
                others = [x for x in weights if x != k and weights[x] < max_single]
                if others:
                    ea = excess / len(others)
                    for o in others: weights[o] = min(max_single, weights[o] + ea)
        ...
        status = "optimal" if max(weights.values()) <= max_single + 0.01 else "feasible"
        return dict(
            strategy_allocations={k: round(v, 4) for k, v in sorted(weights.items(), key=lambda x: -x[1])},
            total_risk=round(total_risk, 4), expected_portfolio_return=round(exp_ret, 4),
            strategy_diversification_score=div, dominant_strategy=dominant, system_status=status,
        )

    # ─── Regime Detector ───

    def market_regime_detector(self, md: dict[str, Any]) -> dict[str, Any]:
        returns = md.get("returns", [0.1, 0.05, -0.02, 0.08, 0.12])
        vol = md.get("volatility", 0.15); spread = md.get("spread_proxy", 0.001)
        dd = abs(md.get("drawdown_pct", 0))
    ...
    def compute_dynamic_rmai(self, base: float, regime: str) -> dict[str, Any]:
        ...
        return dict(dynamic_rmai=round(base * m, 1), multiplier=m, allocation_signal=round(min(base*m, 100), 1))
    ...
    def compute_reality_alignment_index(self, lr: float, sr: float, pr: float) -> dict[str, Any]:
        ...
        ea = round(min(1, sc*0.9+0.1),2); sm = round(0.8 if (sr>=0)==(lr>=0) else 0.4,2)
    ...
    def micro_live_cycle(self, live_return: float) -> dict[str, Any]:
        ...
        return dict(action=act, execution_price=round(ep,2), delay_ms=round(dm,1), slippage_pct=round(sp*100,3),
                    ...
                    pnl_alignment=round(pa,2), kill_switch_triggered=self._kill_switch_active)
